...
def getWeeklyTopRank():
        html = rs.get('http://www.melon.com/chart/week/').text
        soup = BeautifulSoup(html, 'lxml')
        ...
        result.to_csv('~/web_crawler/weekly_top_100.csv', encoding = 'euc-kr')
...
def updateChart(item):
        ...
        for i in range(0,100):
                ...
                query = 'INSERT INTO admin.weeklyTopRank (rank, title, artist, url) VALUES ("%d", "%s", "%s", "%s") ' %(rank, title, artist, url)
                query2 = 'ON DUPLICATE KEY UPDATE rank = "%d", title = "%s", artist = "%s", url = "%s"' %(rank, title, artist, url)
                cursor.execute(query + query2)
                conn.commit()

        conn.close()
        print 'Weekly Top100 DB Update Complete'
